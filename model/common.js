...
const loginParameter = async (model, loginData, res, req) => {
  ...
  user = await db.findSingleDocument(model, { "email": loginData.email, status: loginData.type === 1 ? { $nin: [0, 10, 8] } : { $in: 1 } })
...
//           .collection("sessions")
//           .deleteOne({ "session.userId": logoutData.id });
//         if (error) {
...
//       await mongoose.connection.db
//         .collection("sessions")
//         .deleteOne({ "session.userId": new ObjectId(logoutData.id) });
//       return res.send({
//         status: 1,
//         response: message.logoutSucess,
//       });
